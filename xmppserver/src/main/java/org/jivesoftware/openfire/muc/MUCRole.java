...
/**
 * Defines the permissions and actions that a MUCUser currently may use in a
 * particular room. Each MUCRole defines the relationship between a MUCRoom and
 * a MUCUser.
 *
...
public class MUCRole implements Cacheable, Externalizable {
	...
	/**
	 * Flag that indicates if the room occupant is in the room only to send messages
	 * or also to receive room messages. True means that the room occupant is deaf.
	 */
	...
	/**
	 * The address of the person on the joining FMUC node, if the person joined
	 * through FMUC (otherwise null).
	 */
	...
	/**
	 * This constructor is provided to comply with the Externalizable interface
	 * contract. It should not be used directly.
	 */
	public MUCRole() {
	}
	 ...
	 * Create a new role.
	 *
	 * Note that whenever a new instance of MUCRole is created to reflect a new
	 * relation between an end-user and a MUC room, it is typically needed to
	 * register the same relation by adding the name of the chatroom to the
	 * collection of room names in MUCUser#roomNames. It is the responsibility of
	 * the caller to ensure that this is done.
	 *
	...
	/**
	 * Create a new role that is a 'Room role'. This should never be used to
	 * represent anything else than 'the room itself'.
	 *
	 ...
	 *
	 * The 'from' address of the presence stanza is guaranteed to reflect the room
	 * role of this role.
	 *
	...
	public synchronized void setPresence(Presence newPresence) {
		// Try to remove the element whose namespace is "http://jabber.org/protocol/muc"
		// since we
	...
	/**
	 * Call this method to promote or demote a user's role in a chatroom. It is
	 * common for the chatroom or other chat room members to change the role of
	 * users (a moderator promoting another user to moderator status for example).
	 *
	 * Owning ChatUsers should have their membership roles updated.
	 *
	 * A role is a temporary position or privilege level within a room, distinct
	 * from a user's long-lived affiliation with the room. A role lasts only for the
	 * duration of an occupant's visit to a room.
	 *
	 * @param newRole The new role that the user will play.
	 * @throws NotAllowedException Thrown if trying to change the moderator role to
	 *                             an owner or administrator.
	 */
	public void setRole(MUCRole.Role newRole) throws NotAllowedException {
		// Don't allow to change the role to an owner or admin unless the new role is
		// moderator
		...
		// A moderator cannot be kicked from a room unless there has also been an
		// affiliation change
		if (MUCRole.Role.moderator == role && MUCRole.Role.none == newRole && MUCRole.Affiliation.none != affiliation) {
			throw new NotAllowedException();
		}
		// TODO OF-2288: A moderator MUST NOT be able to revoke voice from a user whose
	...
	/**
	 * Call this method to promote or demote a user's affiliation in a chatroom. An
	 * affiliation is a long-lived association or connection with a room.
	 * Affiliation is distinct from role. An affiliation lasts across a user's
	 * visits to a room.
	 *
	...
	public void setAffiliation(MUCRole.Affiliation newAffiliation) throws NotAllowedException {
		...
		// TODO The fragment is being calculated twice (1. setting the role & 2. setting
		// the aff)
		synchronized (this) {
			calculateExtendedInformation();
		}
	...
	/**
	 * Obtain the affiliation state of the user, which is a long-lived association
	 * or connection with a room. Affiliation is distinct from role. An affiliation
	...
	public MUCRoom getChatRoom() {
		final MultiUserChatService multiUserChatService = XMPPServer.getInstance().getMultiUserChatManager()
				.getMultiUserChatService(roomJid);
		if (multiUserChatService == null) {
			throw new NullPointerException(
					"The MUC service for room '" + roomJid + "' does not exist! This is likely a bug in Openfire.");
		}
	...
	/**
	 * Obtain the XMPPAddress of the user that joined the room. A {@code null} null
	 * value represents the room's role.
	 *
	 * @return The address of the user that joined the room or null if this role
	 ...
	 * This typically represents the XMPPAddress as it is known locally at the
	 * joining FMUC node.
	 *
	 * For users that are joined through FMUC from a remote node, this method will
	 * return the value as reported by the joining FMUC node.
	 *
	 * For users that are in the room, but connected directly to this instance of
	 * Openfire, this method returns null, even if the room is part of an FMUC node.
	 *
	 * Users that joined through server-to-server federation (as opposed to FMUC),
	 * will not have a FMUC address. Null is returned by this method for these
	 * users.
	 *
	 * @return The address of the user that joined the room through FMUC from a
	 *         remote domain, or null.
	 */
	...
	/**
	 * Returns true if the user is one that is in the room as a result of that user
	 * being in another room that is federated with this room, through the FMUC
	 * protocol
	 *
	 * @return true if this user is a user on a remote MUC room that is federated
	 *         with this chatroom.
	 */
	...
	/**
	 * Returns true if the room occupant does not want to get messages broadcast to
	 * all room occupants. This type of users are called "deaf" occupants. Deaf
	 * occupants will still be able to get private messages, presences, IQ packets
	 * or room history.
	 * <p>
	 *
	 ...
	 *
	 * @return true if the room occupant does not want to get messages broadcasted
	 *         to all room occupants.
	 */
	 ...
	 *
	 * Note that sending a packet can modify it (notably, the 'to' address can be
	 * changed). If this is undesired (for example, because post-processing should
	 * not expose the modified 'to' address), then a copy of the original stanza
	 * should be provided as an argument to this method.
	 *
	 * @param packet The packet to send
	 * @see <a href="https://igniterealtime.atlassian.net/browse/OF-2163">issue
	 *      OF-2163</a>
	 */
	public void send(Packet packet) {
		...
		if (this.isRemoteFmuc()) {
			// Sending stanzas to individual occupants that are on remote FMUC nodes defeats
			// the purpose of FMUC, which is to reduce message. This reduction is based on
			// sending data just once, and have it 'fan out' on the remote node (as opposed
			// to sending each occupant on that node a distinct stanza from this node).
			Log.warn(
					"Sending data directly to an entity ({}) on a remote FMUC node. Instead of individual messages, we expect data to be sent just once (and be fanned out locally by the remote node).",
					this, new Throwable());
	...
	/**
	 * When sending data to a user that joined the room through FMUC (when the user
	 * is a user that is local to a remote chatroom that joined our room as a
	 * 'joining FMUC node'), then we'll need to add an 'fmuc' element to all data
	 * that we send it.
	 *
	 * The data that is to be added must include the 'from' address representing the
	 * FMUC role for the occupant that sent the stanza. We either use the reported
	 * FMUC address as passed down from other FMUC nodes, or we use the address of
	 * users connected locally to our server.
	 *
	 * This method will add an 'fmuc' child element to the stanza when the user is a
	 * user that joined through FMUC (is a member of a room that federates with the
	 * MUC room local to our server).
	 *
	 * If the provided stanza already contains an FMUC element with relevant data,
	 * this data is left unchanged.
	 *
	 * @param packet The stanza to augment
	 */
	public void augmentOutboundStanzaWithFMUCData(@Nonnull Packet packet) {
		if (!isRemoteFmuc()) {
			Log.trace("Recipient '{}' is not in a remote FMUC room. No need to augment stanza with FMUC data.",
					this.getUserAddress());
			return;
		}
		Log.trace(
				"Sending data to recipient '{}' that has joined local room '{}' through a federated remote room (FMUC). Outbound stanza is required to include FMUC data: {}",
				this.getUserAddress(), this.getChatRoom().getJID(), packet);

		// Data that was sent to us from a(nother) FMUC node might already have this
		// value. If not, we need to ensure that this is added.
		...
		Log.trace(
				"Adding 'from' FMUC data to outbound stanza, using FMUC address of sender of data, that has been determined to be '{}'.",
				reportingFmucAddress);
	 ...
	 * long-lived affiliation with the room. A role lasts only for the duration of
	 * an occupant's visit to a room.
	 */
	public enum Role {

		/**
		 * Runs moderated discussions. Is allowed to kick users, grant and revoke voice,
		 * etc.
		 */
		moderator(0),

		/**
		 * A normal occupant of the room. An occupant who does not have administrative
		 * privileges; in a moderated room, a participant is further defined as having
		 * voice
		 */
		participant(1),
	...
	/**
	 * A long-lived association or connection with a room. Affiliation is distinct
	 * from role. An affiliation lasts across a user's visits to a room.
	 */
	public enum Affiliation {
		...
		/**
		 * A user who doesn't have an affiliation. This kind of users can register with
		 * members-only rooms and may enter an open room.
		 */
		none(50);
	...
	public String toString() {
		return "MUCRole{" + "roomJid=" + roomJid + ", userJid=" + userJid + ", nick='" + nick + '\'' + ", role=" + role
				+ ", affiliation=" + affiliation + ", voiceOnly=" + voiceOnly + ", rJID=" + rJID + ", reportedFmucJID="
				+ reportedFmucJID + '}';
	}
