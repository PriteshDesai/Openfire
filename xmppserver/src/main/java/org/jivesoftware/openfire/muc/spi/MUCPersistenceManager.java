...
import org.jivesoftware.openfire.custom.dto.GroupData;
...
import org.jivesoftware.openfire.custom.dto.RecallMessage;
import org.jivesoftware.openfire.custom.dto.Relay;
import org.jivesoftware.openfire.custom.dto.SystemNotification;
...
public class MUCPersistenceManager {
	...
	private static final String LOAD_ROOM = "SELECT roomID, creationDate, modificationDate, naturalName, description, lockedDate, "
			...
			+ "fmucInboundNodes " + " FROM ofMucRoom WHERE serviceID=? AND name=?";
	...
	private static final String LOAD_HISTORY = "SELECT sender, nickname, logTime, subject, body, stanza FROM ofMucConversationLog "
			+ "WHERE logTime>? AND roomID=? AND (nickname IS NOT NULL OR subject IS NOT NULL) ORDER BY logTime";
	private static final String RELOAD_ALL_ROOMS_WITH_RECENT_ACTIVITY = "SELECT roomID, creationDate, modificationDate, name, naturalName, description, "
	...
	private static final String LOAD_ALL_AFFILIATIONS = "SELECT ofMucAffiliation.roomID AS roomID, ofMucAffiliation.jid AS jid, ofMucAffiliation.affiliation AS affiliation "
			+ "FROM ofMucAffiliation,ofMucRoom WHERE ofMucAffiliation.roomID = ofMucRoom.roomID AND ofMucRoom.serviceID=?";
	private static final String LOAD_ALL_MEMBERS = "SELECT ofMucMember.roomID AS roomID, ofMucMember.jid AS jid, ofMucMember.nickname AS nickname FROM ofMucMember,ofMucRoom "
			+ "WHERE ofMucMember.roomID = ofMucRoom.roomID AND ofMucRoom.serviceID=?";
	private static final String LOAD_ALL_HISTORY = "SELECT ofMucConversationLog.roomID AS roomID, ofMucConversationLog.sender AS sender, ofMucConversationLog.nickname AS nickname, "
	...
	private static final String UPDATE_ROOM = "UPDATE ofMucRoom SET modificationDate=?, naturalName=?, description=?, "
			...
			+ "fmucOutboundNode=?, fmucOutboundMode=?, fmucInboundNodes=? " + "WHERE roomID=?";
	...
	/*
	 * This is the query to store messages into database for offline user.
	 */
	...
	/*
	 * This SQL query is to get the user device details in the group
	 */
	...
	/*
	 * This SQL query is used to get user from some room and check for its
	 * notification status
	 */
	...
	/**
	 * Returns the reserved room nickname for the bare JID in a given room or null
	 * if none.
	 *
	 * @param room    the room where the user would like to obtain his reserved
	 *                nickname.
	 * @param bareJID The bare jid of the user of which you'd like to obtain his
	 *                reserved nickname.
	 ...
	 *
	 * Note that this method will count only rooms that are persisted in the
	 * database, and can exclude in-memory rooms that are not persisted.
	 *
	...
	public static int countRooms(MultiUserChatService service) {
		...
		try {
			Long serviceID = XMPPServer.getInstance().getMultiUserChatManager()
					.getMultiUserChatServiceID(service.getServiceName());
			con = DbConnectionManager.getConnection();
			...
			if (!rs.next()) {
				throw new IllegalArgumentException(
						"Service " + service.getServiceName() + " was not found in the database.");
			}
			return rs.getInt(1);
		} catch (SQLException sqle) {
			Log.error("An exception occurred while trying to count all persisted rooms of service '{}'",
					service.getServiceName(), sqle);
			return -1;
		} finally {
			DbConnectionManager.closeConnection(rs, pstmt, con);
		}
	...
	public static void loadFromDB(MUCRoom room) {
		...
		try {
			...
			if (rs.getString("fmucOutboundNode") != null) {
				...
				default:
				case 0:
					fmucOutboundJoinMode = MasterMaster;
					break;
				case 1:
					fmucOutboundJoinMode = FMUCMode.MasterSlave;
					break;
				}
			...
			if (rs.getString("fmucInboundNodes") != null) {
				final Set<JID> fmucInboundNodes = Stream.of(rs.getString("fmucInboundNodes").split("\n"))
						.map(String::trim).map(JID::new).collect(Collectors.toSet());
				// A list, which is an 'allow only on list' configuration. Note that the list
				// can be empty (effectively: disallow all).
			...
			// Set now that the room's configuration is updated in the database. Note: We
			// need to
			// set this now since otherwise the room's affiliations will be saved to the
			// database
	...
	public static void saveToDB(MUCRoom room) {
		...
		try {
			...
			if (room.wasSavedToDB()) {
				Log.info("*************** createQR Inside Else Update room is called");
				...
				// Store a newline-separated collection, which is an 'allow only on list'
				// configuration. Note that the list can be empty (effectively: disallow all),
				// or null: this is an 'allow all' configuration.
				if (room.getFmucInboundNodes() == null) {
					pstmt.setNull(21, Types.VARCHAR); // Null: allow all.
				} else {
					final String content = room.getFmucInboundNodes().stream().map(JID::toString)
							.collect(Collectors.joining("\n")); // result potentially is an empty String, but will not
																// be null.
				...
				Log.info("*************** createQR Inside Else Add room is called");
	...
	public static void createQR(String roomId) throws WriterException, IOException {
		Log.info("*************** createQR called");
		// The data that the QR code will contain
		String data = "https://gatherhall.com/download?roomId=" + roomId;
		Log.info("*************** data" + data);

		// The path where the image will get saved
// 		String localPath = "/home/jayesh_prajapati/Downloads/" + roomId + ".jpg";
		String path = "/usr/share/openfire/conf/roomqrcodes/" + roomId + ".jpg";
		Log.info("*************** path" + path);
		...
		Log.info("*************** matrix" + matrix.toString());

		Log.info("path.substring(path.lastIndexOf('.') + 1) : " + path.substring(path.lastIndexOf('.') + 1));
		Log.info("path.substring(path.lastIndexOf('.') + 1) : " + new File(path).toPath());
		...
		Log.info("*************** After Write QR CODE");

		Log.info(roomId + " :: QR Code Generated!!! ");
	}
			...
			try {
				...
				while (resultSet.next()) {
					try {
						...
						Log.error(
								"A database exception prevented one particular MUC room name to be loaded from the database.",
								e);
					}
				}
			} finally {
	 ...
	 * This query will be executed only when the service is starting up.
	 *
	 * @param chatserver  the chat server that will hold the loaded rooms.
	 * @param cleanupDate rooms that hadn't been used after this date won't be
	 *                    loaded.
	...
	private static Map<Long, MUCRoom> loadRooms(Long serviceID, Date cleanupDate, MultiUserChatService chatserver)
			throws SQLException {
			...
			while (resultSet.next()) {
				try {
					...
					String roles = StringUtils
							.zeroPadString(Integer.toBinaryString(resultSet.getInt("rolesToBroadcast")), 3);
					if (roles.charAt(0) == '1') {
						rolesToBroadcast.add(MUCRole.Role.moderator);
					}
					if (roles.charAt(1) == '1') {
	...
	/**
	 * Load or reload the room history for a particular room from the database into
	 * memory.
	 *
	 * Invocation of this method will replace exising room history that's stored in
	 * memory (if any) with a freshly set of messages obtained from the database.
	 *
	 * @param room The room for which to load message history from the database into
	 *             memory.
	 * @throws SQLException
	 */
	 ...
	 * memory (if any) with a freshly set of messages obtained from the database.
	 *
	 * @param room      The room for which to load message history from the database
	 *                  into memory.
	 * @param maxNumber A hint for the maximum number of messages that need to be
	 *                  read from the database. -1 for all messages.
	 * @throws SQLException
	 */
	public static void loadHistory(@Nonnull final MUCRoom room, final int maxNumber) throws SQLException {
		...
		try {
			if (room.isLogEnabled()) {
				con = DbConnectionManager.getConnection();
				pstmt = con.prepareStatement(LOAD_HISTORY, ResultSet.TYPE_SCROLL_INSENSITIVE,
				...
				// When reloading history, make sure that the old data is removed from memory
				// before re-adding it.
				...
				while (rs.next()) {
					...
					oldMessages.add(room.getRoomHistory().parseHistoricMessage(senderJID, nickname, sentDate, subject,
							body, stanza));
				}
			...
			// If the room does not include the last subject in the history then recreate
			// one if
			// possible
			if (!room.getRoomHistory().hasChangedSubject() && room.getSubject() != null
					&& room.getSubject().length() > 0) {
				final Message subject = room.getRoomHistory().parseHistoricMessage(
						room.getRole().getRoleAddress().toString(), null, room.getModificationDate(), room.getSubject(),
						null, null);
	...
	private static void loadHistory(Long serviceID, Map<Long, MUCRoom> rooms) throws SQLException {
		...
		try {
			...
			while (resultSet.next()) {
				try {
					...
					// Skip to the next position if the room does not exist or if history is
					// disabled
					...
					final Message message = room.getRoomHistory().parseHistoricMessage(senderJID, nickname, sentDate,
							subject, body, stanza);
		...
		// Add the last known room subject to the room history only for those rooms that
		// still
		...
		for (MUCRoom loadedRoom : rooms.values()) {
			if (!loadedRoom.getRoomHistory().hasChangedSubject() && loadedRoom.getSubject() != null
				...
				final Message message = loadedRoom.getRoomHistory().parseHistoricMessage(
						loadedRoom.getRole().getRoleAddress().toString(), null, loadedRoom.getModificationDate(),
						loadedRoom.getSubject(), null, null);
		...
		try {
			...
			while (resultSet.next()) {
				try {
					...
					final MUCRole.Affiliation affiliation = MUCRole.Affiliation
							.valueOf(resultSet.getInt("affiliation"));
	 ...
	 * information will be saved only if the room is_persistent and has already been
	 * saved to the database previously.
	 * 
	 ...
	 * @param nickname       The reserved nickname of the user in the room or null
	 *                       if none.
	 ...
	 * persistent.
	 *
	 * @param affiliationJID The bareJID of the user to remove his affiliation from
			...
			for (ConversationLogEntry entry : batch) {
				...
				try {
					...
					if (message.getSubject() != null && (message.getSubject().equalsIgnoreCase("1")
						...
						groupMessagepstm.setInt(1, new Long(entry.getRoomID()).intValue());
						groupMessagepstm.setInt(2, new Long(entry.getRoomID()).intValue());
						...
						if (groupMessageRS != null) {
							...
							/*
							 * Itterate loop through all user device of the group and check for
							 * notifications
							 */
							while (groupMessageRS.next()) {
								Log.info("GroupChat 1.6");
								checkNotiRes.setString(1, groupMessageRS.getString("username"));
								checkNotiRes.setInt(2, new Long(entry.getRoomID()).intValue());
					...
					if (message.getSubject() != null && (message.getSubject().equalsIgnoreCase("1")
						...
						try {
							if (userDeviceRoasterDetails != null && userDeviceRoasterDetails.size() > 0) {
								...
								/*
								 * Here we are setting android body and ios body both Because in a room there
								 * may be both android device and ios device
								 */
								if (gifCount > 0) {
									if (androidBody != null) {
										if (gifCount > 1) {
	...
	private static int marshallRolesToBroadcast(MUCRoom room) {
		final String buffer = (room.canBroadcastPresence(MUCRole.Role.moderator) ? "1" : "0")
	 ...
	 * exist, the {@code defaultValue} will be returned.
	 *
	 * @param subdomain    the subdomain of the service to retrieve a property from
	 * @param name         the name of the property to return.
	 * @param defaultValue value returned if the property doesn't exist or was not a
	 *                     number.
	...
	/**
	 * Returns a long value Jive property. If the specified property doesn't exist,
	 * the {@code defaultValue} will be returned.
	 *
	 * @param subdomain    the subdomain of the service to retrieve a property from
	 * @param name         the name of the property to return.
	 * @param defaultValue value returned if the property doesn't exist or was not a
	 ...
	 * @param subdomain the subdomain of the service to retrieve a property from
	 * @param name      the name of the property to return.
	 * @return true if the property value exists and is set to {@code "true"}
	 *         (ignoring case). Otherwise {@code false} is returned.
	 */
	...
	/**
	 * Return all immediate children property names of a parent Jive property as a
	 * list of strings, or an empty list if there are no children. For example,
	 * given the properties {@code X.Y.A}, {@code X.Y.B}, {@code X.Y.C} and
	 * {@code X.Y.C.D}, then the immediate child properties of {@code X.Y} are
	 * {@code A}, {@code B}, and {@code C} ({@code C.D} would not be returned using
	 * this method).
	 * <p>
	 *
	 * @param subdomain the subdomain of the service to retrieve a property from
	...
	/**
	 * Return all immediate children property values of a parent Jive property as a
	 * list of strings, or an empty list if there are no children. For example,
	 * given the properties {@code X.Y.A}, {@code X.Y.B}, {@code X.Y.C} and
	 * {@code X.Y.C.D}, then the immediate child properties of {@code X.Y} are
	 * {@code X.Y.A}, {@code X.Y.B}, and {@code X.Y.C} (the value of {@code X.Y.C.D}
	...
	/**
	 * Sets a Jive property. If the property doesn't already exists, a new one will
	 * be created.
	 *
	...
	/**
	 * Sets multiple Jive properties at once. If a property doesn't already exists,
