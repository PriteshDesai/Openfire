 ...
 */
package org.jivesoftware.openfire.com.huawei.push.apns;
...
public class ApnsHeaders {
	...
	@JsonProperty(value = "apns-id")
	private String apnsId;

	@JsonProperty(value = "apns-expiration")
	private Long apnsExpiration;

	@JsonProperty(value = "apns-priority")
	private String apnsPriority;

	@JsonProperty(value = "apns-topic")
	...
	public void check() {
		if (this.authorization != null) {
			ValidatorUtils.checkArgument(this.authorization.matches(AUTHORIZATION_PATTERN),
		...
		if (this.apnsPriority != null) {
			ValidatorUtils.checkArgument(
					Integer.parseInt(this.apnsPriority) == SEND_BY_GROUP
							|| Integer.parseInt(this.apnsPriority) == SEND_IMMEDIATELY,
					"apns-priority should be SEND_BY_GROUP:5  or SEND_IMMEDIATELY:10");
		}
		if (this.apnsCollapseId != null) {
			ValidatorUtils.checkArgument(this.apnsCollapseId.getBytes().length < 64,
