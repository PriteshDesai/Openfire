...
public class HuaweiApp {
	...
	/**
	 * HuaweiMessaging can be added in the pattern of service, whcih is designed for
	 * Scalability
	 */
	...
	/**
	 * Returns the instance identified by the unique appId, or throws if it does not
	 * exist.
	 *
	 * @param appId represents the id of the {@link HuaweiApp} instance.
	 * @return the {@link HuaweiApp} corresponding to the id.
	 * @throws IllegalStateException if the {@link HuaweiApp} was not initialized,
	 *                               either via {@link #initializeApp(HuaweiOption)}
	 *                               or {@link #getApps()}.
	 */
...
//            throw new IllegalStateException(errorMessage);
	 ...
	 *
	 * @throws IllegalStateException if the app instance has already been
	 *                               initialized.
	 */
	public static HuaweiApp initializeApp(HuaweiOption option) {
		...
		synchronized (appsLock) {
			if (!instances.containsKey(appId)) {
				ValidatorUtils.checkState(!instances.containsKey(appId),
						"HuaweiApp with id " + appId + " already exists!");
	...
	/**
	 * Deletes the {@link HuaweiApp} and all its data. All calls to this
	 * {@link HuaweiApp} instance will throw once it has been called.
	 *
	 * <p>
	...
	ScheduledFuture<?> schedule(Runnable runnable, long initialDelay, long period) {
		return getScheduledExecutorService().scheduleWithFixedDelay(runnable, initialDelay, period,
				TimeUnit.MILLISECONDS);
	}

	/**
	 * Add service to the app, such as HuaweiMessaging, other services can be added
	 * if needed
	 */
