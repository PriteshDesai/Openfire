...
public class AndroidNotification {
	...
	@JsonProperty(value = "multi_lang_key")
	private JsonObject multiLangKey;

	@JsonProperty(value = "channel_id")
	private String channelId;

	@JsonProperty(value = "notify_summary")
	...
	@JsonProperty(value = "big_title")
	private String bigTitle;

	@JsonProperty(value = "big_body")
	...
	@JsonProperty(value = "auto_cancel")
	private boolean autoCancel;

	@JsonProperty(value = "when")
	private String when;

	@JsonProperty(value = "local_only")
	private Boolean localOnly;

	@JsonProperty(value = "importance")
	private String importance;

	@JsonProperty(value = "use_default_vibrate")
	private boolean useDefaultVibrate;

	@JsonProperty(value = "use_default_light")
	private boolean useDefaultLight;

	@JsonProperty(value = "vibrate_config")
	private List<String> vibrateConfig = new ArrayList<>();

	@JsonProperty(value = "visibility")
	private String visibility;

	@JsonProperty(value = "light_settings")
	private LightSettings lightSettings;

	@JsonProperty(value = "foreground_show")
	private boolean foregroundShow;

	@JsonProperty(value = "inbox_content")
	...
	public void check(Notification notification) {
		if (null != notification) {
			ValidatorUtils.checkArgument(
					StringUtils.isNotEmpty(notification.getTitle()) || StringUtils.isNotEmpty(this.title),
					"title should be set");
			ValidatorUtils.checkArgument(
					StringUtils.isNotEmpty(notification.getBody()) || StringUtils.isNotEmpty(this.body),
					"body should be set");
		}
		...
		if (!CollectionUtils.isEmpty(this.bodyLocArgs)) {
			ValidatorUtils.checkArgument(StringUtils.isNotEmpty(this.bodyLocKey),
					"bodyLocKey is required when specifying bodyLocArgs");
		}

		if (!CollectionUtils.isEmpty(this.titleLocArgs)) {
			ValidatorUtils.checkArgument(StringUtils.isNotEmpty(this.titleLocKey),
		...
		// Style 0,1,2
		if (this.style != null) {
			boolean isTrue = this.style == 0 || this.style == 1 || this.style == 3;
			ValidatorUtils.checkArgument(isTrue, "style should be one of 0:default, 1: big text, 3: Inbox");

			if (this.style == 1) {
				ValidatorUtils.checkArgument(
						StringUtils.isNotEmpty(this.bigTitle) && StringUtils.isNotEmpty(this.bigBody),
						"title and body are required when style = 1");
			} else if (this.style == 3) {
				ValidatorUtils.checkArgument(
						!CollectionUtils.isEmpty(this.inboxContent) && this.inboxContent.size() <= 5,
		...
		if (this.importance != null) {
			ValidatorUtils.checkArgument(
					StringUtils.equals(this.importance, Importance.LOW.getValue())
		...
		if (!CollectionUtils.isEmpty(this.vibrateConfig)) {
			ValidatorUtils.checkArgument(this.vibrateConfig.size() <= 10,
					"vibrate_config array size cannot be more than 10");
			for (String vibrateTiming : this.vibrateConfig) {
				ValidatorUtils.checkArgument(vibrateTiming.matches(AndroidNotification.VIBRATE_PATTERN),
						"Wrong vibrate timing format");
		...
		if (this.visibility != null) {
			ValidatorUtils.checkArgument(
					StringUtils.equals(this.visibility, Visibility.VISIBILITY_UNSPECIFIED.getValue())
