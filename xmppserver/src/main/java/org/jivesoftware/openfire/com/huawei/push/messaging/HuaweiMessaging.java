 ...
 * by a instance of {@link com.huawei.push.messaging.HuaweiApp}, and then use it
 * to send a message
 */
public class HuaweiMessaging {
	...
	/**
	 * Gets the {@link HuaweiMessaging} instance for the specified
	 * {@link HuaweiApp}.
	 *
	 * @return The {@link HuaweiMessaging} instance for the specified
	 *         {@link HuaweiApp}.
	 */
	public static synchronized HuaweiMessaging getInstance(HuaweiApp app) {
		HuaweiMessagingService service = ImplHuaweiTrampolines.getService(app, SERVICE_ID,
				HuaweiMessagingService.class);
		if (service == null) {
	...
	private static HuaweiMessaging fromApp(final HuaweiApp app) {
		return HuaweiMessaging.builder().setApp(app).setMessagingClient(() -> HuaweiMessageClientImpl.fromApp(app))
				.build();
	}
	 ...
	 * @throws HuaweiMesssagingException If an error occurs while handing the
	 *                                   message off to HCM for delivery.
	 */
	...
	public SendResponse subscribeTopic(TopicMessage topicMessage) throws HuaweiMesssagingException {
		...
		return messagingClient.send(topicMessage, TopicOperation.SUBSCRIBE.getValue(),
				ImplHuaweiTrampolines.getAccessToken(app));
	}
	...
	public SendResponse unsubscribeTopic(TopicMessage topicMessage) throws HuaweiMesssagingException {
		final HuaweiMessageClient messagingClient = getMessagingClient();
		return messagingClient.send(topicMessage, TopicOperation.UNSUBSCRIBE.getValue(),
				ImplHuaweiTrampolines.getAccessToken(app));
	}
	...
	public SendResponse listTopic(TopicMessage topicMessage) throws HuaweiMesssagingException {
		final HuaweiMessageClient messagingClient = getMessagingClient();
		return messagingClient.send(topicMessage, TopicOperation.LIST.getValue(),
				ImplHuaweiTrampolines.getAccessToken(app));
	}
	 ...
	 *
	 * <p>
	 * If the {@code validateOnly} option is set to true, the message will not be
	 * actually sent. Instead HCM performs all the necessary validations, and
